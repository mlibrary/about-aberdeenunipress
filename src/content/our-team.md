---
templateKey: our-team-page
title: Aberdeen University Press Team
groups:
  - groupTitle: Aberdeen University Press
    columnClass: col-md-4
    note: "Contact us: [aup@abdn.ac.uk](mailto:aup@abdn.ac.uk)"
    members:
      - name: Emma Francis
        role: Aberdeen University Press Manager
        image: /assets/francis-300px.png
        imgFluid: true
      - name: Sandra Hynes
        role: Aberdeen University Press Administrator
        image: /assets/hynes-300px.png
        imgFluid: false
      - name: Varina Jones-Reid
        role: Open Research & Metadata Assistant
        image: /assets/jonesreid-300px.png
        imgFluid: false
  - groupTitle: AUP Executive Board
    columnClass: col-md-3
    members:
      - name: Simon Bains
        role: University Librarian
        image: /assets/bains-300px.png
        imgFluid: false
      - name: Michael Brown
        role: Chair in Irish, Scottish and Enlightenment History
        image: /assets/brown-300px.png
        imgFluid: false
      - imgFluid: false
        name: Neil Curtis
        role: Head of University Collections
        image: assets/Neil_photo_square.jpg
      - name: Andrew Dilley
        role: Chair of the Friends of Aberdeen University Library, School of Divinity,
          History, Philosophy and Art History
        image: /assets/dilley-300px.png
        imgFluid: true
      - name: Emma Francis
        role: Aberdeen University Press and Open Research Manager
        image: /assets/francis-300px.png
        imgFluid: false
      - name: Nir Oren
        role: Dean for Research Performance & Interdisciplinary Institute Director - AI
          & Big Data
        image: /assets/oren-200px.png
        imgFluid: true
      - name: Ian J. Robotham
        role: Director, Digital & Information Services
        subRole: Chair of AUP Executive Board
        image: /assets/robotham-300px.png
        imgFluid: true
      - imgFluid: false
        name: Ben Tatler
        role: Vice Principal for Research
        image: assets/tatler-300px.png
      - name: Charles Watkinson
        role: Director, University of Michigan Press
        image: /assets/watkinson-300px.png
        imgFluid: false
      - name: John Wrighton
        role: Head of Research Strategy & Policy & REF Manager
        image: /assets/wrighton-300px.png
        imgFluid: false
  - groupTitle: Academic Advisory Board
    subtitle: For the period 2025-2028
    columnClass: col-md-3
    members:
      - name: Simon Bains
        role: University Librarian
        image: /assets/bains-300px.png
        imgFluid: false
      - name: Michael Beaney
        role: School of Divinity, History, Philosophy and Art History
        image: /assets/beaney-300px.png
        imgFluid: false
      - name: Anshuman Bhardwaj
        role: School of Geosciences
        image: /assets/bhardwaj-300px.png
        imgFluid: true
      - name: Michael Brown
        role: Chair, School of Divinity, History, Philosophy and Art History
        subRole: Chair of the Academic Advisory Board
        image: /assets/brown-300px.png
        imgFluid: true
      - name: Vikki Entwistle
        role: School of Medicine, Medical Sciences and Nutrition
        image: /assets/entwistle-300px.png
        imgFluid: true
      - name: Emma Francis
        role: Aberdeen University Press Manager
        image: /assets/francis-300px.png
        imgFluid: true
      - name: Sandra Hynes
        role: Aberdeen University Press Administrator
        image: /assets/hynes-300px.png
        imgFluid: false
      - name: Rebecca Macklin
        role: School of Language, Literature, Music and Visual Culture
        image: /assets/macklin-300px.png
        imgFluid: false
      - name: Clare Moran
        role: School of Law
        image: /assets/moran-300px.png
        imgFluid: false
      - name: Juliano Morimoto
        role: School of Natural and Computing Sciences
        image: /assets/morimoto-300px.png
        imgFluid: true
      - name: Joseph Pierce
        role: School of Geosciences
        image: /assets/pierce-200px.png
        imgFluid: true
      - name: Rainer Schulz
        role: Business School
        image: /assets/schulz-300px.png
        imgFluid: false
      - name: Rachel Shanks
        role: School of Education
        image: /assets/shanks-300px.png
        imgFluid: false
---
